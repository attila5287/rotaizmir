...
let filtered = { a: "admin", m: "member", p: "prez", };
let categories = [
  'm',
  'a',
  'p',
];
let styles = { a: "secondary", m: "primary", p: "info", }; 


d3.selectAll( ".filter_btn" )
  .on( 'click', () => {

    activate_filter_buttons();
  });
...
function activate_filter_buttons () {
  ...
  categories.forEach( ( category ) => {
    ...
    if ( category == selected ) {
      ...
      selected_box.select( '.filter_btn' ).on( 'click', () => activate_filter_buttons() );
    ...
    if (new_categories != '') {
      ...
      d3.select("#filter_submit_box")
        ...
        .attr(
          "class",
          "filter_submit_btn btn btn-outline-primary btn-block text-lg text-comfo shadow mt-0"
        )
      ...
      d3.select("#filtered_categories")
        .attr("class", "text-italic text-center mb-0 mt-2")
        .text(default_msg);
    } else {
      const warning_msg = "No Categories Included!";
      d3.select( "#filtered_categories" )
        .attr('class', 'text-secondary mb-0 mt-2')
        .text( warning_msg );
      let warning_button = d3.select( "#filter_submit_box" )
        .append( "a" )
        .attr(
          "class",
          "filter_submit_btn btn btn-secondary btn-block text-lg text-comfo shadow mt-0 disabled"
      ); 
      warning_button
        .append( "i" )
